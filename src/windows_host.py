...
#  we will call this function if usb device is connected with system 
    ...
    def __usb_list__(self):
        ...
        print("Hello")
        out = subprocess.getoutput("PowerShell -Command \"& {Get-PnpDevice -PresentOnly | Where-Object { $_.InstanceId -match '^USB' } | Select-Object Status,Class,FriendlyName,InstanceId,Manufacturer | ConvertTo-Json}\"")
        j = json.loads(out)
        for dev in j:
